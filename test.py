...
import requests
from param import APP_VERSION, APP_URL, UPDATE_JSON_URL
...
class UpdateCheckWorker(QObject):
    ...
    def run(self):
        try:
            meta = _fetch_update_metadata(self.current_version)
...
def _fetch_update_metadata(current_version):
    headers = {"User-Agent": f"ClientFlow/{current_version}"}

    # 1) Primary source: hosted version.json
    try:
        r = requests.get(UPDATE_JSON_URL, timeout=10, headers=headers)
    ...
    try:
        ...
        if not latest_version:
            raise Exception("Impossible de lire la version de la release.")
        asset_url = _pick_asset(release.get("assets", [])) or APP_URL
        return {
    ...
    # 3) Last resort: direct download URL without version check
    if APP_URL:
        return {
            "version": "",
            "notes": "",
    ...
    raise Exception("Impossible de vérifier les mises à jour.")


def _on_update_check_finished(self, meta):
    ...
    try:
        ...
        if not latest_version:
            QMessageBox.warning(self, "Erreur", "Impossible de lire la version disponible.")
            return
